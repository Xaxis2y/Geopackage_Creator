...
"""Network/internet check helpers for the DGIWG validator (v1.60)."""
import os
import sys
import json
import re
...
import urllib.error
# v1.57 fix (Bug #2): ET was referenced by _net_check_dmf_xml() but never
# imported in this module — every call raised NameError, so the ISO 19115/DMF
# internet check had never produced a real result.
...
# Path to the bundled EPSG cache file.
# v1.57 fix (#10): resolved relative to the package location (__file__) instead
# of sys.argv[0], which broke when the tool was invoked through a wrapper,
# console-script entry point, or symlink.  The cache ships in the project root
# (one level above the dgiwg_validator package, next to the launcher script);
# the package folder itself is checked as a secondary location.
_EPSG_CACHE_CANDIDATES = [
    os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))),
                 "dgiwg_epsg_cache.json"),
    os.path.join(os.path.dirname(os.path.abspath(__file__)),
                 "dgiwg_epsg_cache.json"),
]
_EPSG_JSON_CACHE_PATH = next(
    (p for p in _EPSG_CACHE_CANDIDATES if os.path.isfile(p)),
    _EPSG_CACHE_CANDIDATES[0],
)
...
def _net_get(url, timeout=None):
    ...
    """
    if timeout is None:
        timeout = _constants.NET_TIMEOUT
    if _config.OFFLINE:
...
def _net_check_uri(uri, timeout=None):
    """Returns (ok:bool, note:str). ok=True if reachable."""
    # v1.58 fix: defense-in-depth — this helper built its own urllib request
    # and bypassed the --offline gate in _net_get().  The only current call
    # site is already guarded, but any future caller must not leak network
    # I/O on air-gapped systems.
    if _config.OFFLINE:
        return (None, "OFFLINE MODE — URI reachability check disabled (--offline flag)")
    if timeout is None:
        timeout = _constants.NET_TIMEOUT
...
def _net_check_epsg(srs_id, srs_name, timeout=None):
...
def _net_check_scale_denoms(srs_id, zoom_pixel_sizes, tolerance=0.005, timeout=None):
    """
...
def _http_get(url, timeout=None):
    ...
    v1.60 fix: this helper built its own urllib request and so bypassed the
    --offline gate in _net_get(), exactly like _net_check_uri() did before the
    v1.58 fix.  Any caller added later would have leaked network I/O on
    air-gapped / 국방망 systems.  Gated here as defence-in-depth.
    """
    if _config.OFFLINE:
        return False, "OFFLINE MODE — internet checks disabled (--offline flag)"
    if timeout is None:
        timeout = _constants.NET_TIMEOUT
    try:
...
def _http_head(url, timeout=None):
    ...
    v1.60 fix: --offline gate added — see _http_get() above.
    """
    if _config.OFFLINE:
        return False, "OFFLINE MODE — internet checks disabled (--offline flag)"
    if timeout is None:
