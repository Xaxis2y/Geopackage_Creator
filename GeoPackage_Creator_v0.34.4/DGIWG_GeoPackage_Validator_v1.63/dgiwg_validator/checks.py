...
"""Requirement check functions for DGIWG STD-DP-19-005 v1.1 (v1.60).
...
def check_req(cursor, req_num):
    ...
    try:
        ...
        return ("FAIL", f"Exception during check: {e}", None)
...
def _r9(cursor):
    ...
    # v1.61: every row in `rows` was checked against the allowlist above — this
    # check is exhaustive over the file's 2D vector geometry columns, so a
    # clean result is a genuine PASS, not a coverage-limited PASS*.
...
def _r10(cursor):
    ...
    # v1.61: exhaustive over `rows` — same reasoning as _r9.
    return ("FAIL" if fails else "PASS", "; ".join(fails + passes))
...
def _r15(cursor):
    ...
    for srs_id, srs_name, wkt in compound_entries:
        ...
        # v1.60: redundant `import re as _re` removed — already imported at the
