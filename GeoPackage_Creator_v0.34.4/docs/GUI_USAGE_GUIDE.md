...
**Source FileGDB**: Select the geodatabase folder. A `.gdb` suffix is
conventional but not required when `GEODATABASE_FILE_*` markers are present.
...
- See `INSTALLATION_GUIDE.md` and `GDAL_INSTALLATION.txt` for setup help, and
  `VERSION.txt` / `CHANGELOG.md` for release history.
