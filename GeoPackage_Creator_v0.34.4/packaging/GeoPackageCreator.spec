...
# ======================
# PyInstaller spec for GeoPackage Creator v0.34.4
#
...
# ---------------------------------------------------------------------------
# DGIWG validator folder: through v0.30.16 this was
# DGIWG_Validator_v1_55_updated; v0.30.17 renamed it to
# DGIWG_GeoPackage_Validator_v1.62, and it was renamed AGAIN in place to
# DGIWG_GeoPackage_Validator_v1.63 with no application code change. Both
# renames broke a hardcoded literal here the same way: PyInstaller's
# Analysis() fails outright with a "source path does not exist" error the
# moment the folder name in the tuple below stops matching what is actually
# on disk. v0.34.3 fixes this at the root by discovering the folder name via
# glob instead of hardcoding it (below), so the NEXT validator version bump
# does not require touching this spec file again. This folder is
# intentionally NOT in hiddenimports: core/validation_gate.py's
# find_validator() and packaging/runtime_paths.py's bootstrap() both locate
# and load it at RUNTIME via sys.path.insert() + `from dgiwg_validator import
# checks` against a real on-disk directory next to the frozen exe
# (sys._MEIPASS for one-file, the app folder for one-dir) - a dynamic import
# PyInstaller's static analyzer cannot see and does not need to, since the
# files are plain .py source copied as data, not compiled into the PYZ.
# Bundling it here as data, at the SAME relative path the source tree
# already uses, is what lets runtime_paths.bootstrap()'s glob-based
# discovery find it unchanged inside a frozen build.
_validator_dirs = sorted(ROOT.glob("DGIWG_GeoPackage_Validator_v1.*"))
if len(_validator_dirs) != 1:
    raise SystemExit(
        "Expected exactly one DGIWG_GeoPackage_Validator_v1.* folder next to "
        f"{ROOT}, found: {[d.name for d in _validator_dirs]}. Remove the "
        "stale copy (or add the missing one) before building."
    )
DGIWG_VALIDATOR_DIR_NAME = _validator_dirs[0].name
...
app_datas = [
    ...
    (str(ROOT / DGIWG_VALIDATOR_DIR_NAME),               DGIWG_VALIDATOR_DIR_NAME),
...
hidden = [
    ...
    *collect_submodules("osgeo"),
    # Geometry / projection
    # v0.34.3: "shapely"/"shapely.geometry"/collect_submodules("shapely")
    # removed -- nothing in core/, packaging/, or the CLI/GUI entry points
    # imports shapely, it is not in requirements.txt or environment.yml,
    # and PyInstaller logged "ERROR: Hidden import 'shapely' not found" /
    # "ERROR: Hidden import 'shapely.geometry' not found" on every build
    # because of it (harmless -- PyInstaller doesn't fail the build over a
    # missing *hidden* import -- but it's ERROR-level noise in a log that's
    # supposed to be trustworthy at a glance).
    "pyproj",
    ...
    *collect_submodules("ttkbootstrap"),
    ...
    # DGIWG_VALIDATOR_DIR_NAME/, see the app_datas comment above) is
