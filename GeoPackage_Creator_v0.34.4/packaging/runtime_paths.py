...
def bootstrap() -> Path:
    ...
    # explicitly to the bundled folder when present. v0.34.3: the previous
    # fast path hardcoded "DGIWG_GeoPackage_Validator_v1.62" ahead of the
    # glob fallback -- harmless while v1.62 was current, but misleading (and
    # a future footgun if an old copy is ever left alongside a newer one)
    # once the bundled validator was upgraded in place to v1.63. Sort all
    # DGIWG_GeoPackage_Validator_v1.* matches and take the highest version so
    # this always prefers the newest bundled copy without needing an
    # application code change on the next validator bump.
    if not os.environ.get("DGIWG_VALIDATOR_PATH"):
        ...
        validator_dir = _first_existing(
            new_style[-1] if new_style else None,
