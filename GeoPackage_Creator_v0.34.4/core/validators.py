...
class InputValidator:
    ...
    @staticmethod
    def resolve_source_path(file_path: str) -> str:
        """Resolve a downloaded FileGDB container to its single .gdb child.

        Some CANVEC downloads use a descriptive parent folder containing
        ``GEODATABASE_FILE_*\<dataset>.gdb``.  GDAL cannot open that parent
        directly even though it is a valid user-selected distribution folder.
        Preserve ordinary paths and only unwrap an unambiguous single dataset.
        """
        path = Path(file_path)
        if path.is_dir() and path.suffix.lower() != ".gdb":
            candidates = sorted(path.glob("GEODATABASE_FILE_*/*.gdb"))
            if len(candidates) == 1:
    ...
    def validate_source_file(file_path: str) -> bool:
        ...
        """
        file_path = InputValidator.resolve_source_path(file_path)
        path = Path(file_path)
