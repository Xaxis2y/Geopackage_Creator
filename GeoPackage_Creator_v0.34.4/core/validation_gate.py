...
GeoPackage Validator (version discovered at runtime, see VERSION.txt in
the validator folder) against the freshly created
...
3. Well-known sibling locations relative to this installation
   (e.g. DGIWG_GeoPackage_Validator_v1.x)
...
def _find_validator_launcher(vdir: Path) -> Path:
    """Return the single ``DGIWG_Validator_v1_*.py`` launcher inside *vdir*.

    v0.34.3: this used to be a hardcoded ``DGIWG_Validator_v1_62.py``. The
    bundled validator is upgraded in place on its own schedule, independent
    of this app's version (v1.62 -> v1.63 happened without any code change
    here), and a hardcoded filename breaks --validate outright the moment
    that happens: find_validator() still locates the (correctly upgraded)
    folder, but the launcher lookup inside it silently no longer exists.
    Discovering it by pattern instead means an in-place validator upgrade
    never requires an application code change to keep working.
    """
    candidates = sorted(vdir.glob("DGIWG_Validator_v1_*.py"))
    if not candidates:
        raise FileNotFoundError(
            f"No DGIWG_Validator_v1_*.py launcher found in {vdir}"
        )
    if len(candidates) > 1:
        raise FileNotFoundError(
            f"Multiple DGIWG_Validator_v1_*.py launchers found in {vdir}: "
            f"{[c.name for c in candidates]} -- ambiguous, remove the stale one"
        )
    return candidates[0]
...
def run_dgiwg_validation(
    ...
    try:
        ...
        launcher = _find_validator_launcher(vdir)

        with tempfile.TemporaryDirectory(prefix="gpkg_dgiwg_") as report_dir:
            env = os.environ.copy()
            if getattr(sys, "frozen", False):
                command = [sys.executable, "--dgiwg-validator-worker"]
                # v0.34.3: tell the frozen worker exactly which validator
                # directory find_validator() resolved above (an explicit
                # --validator-path / DGIWG_VALIDATOR_PATH override, or a
                # discovered sibling install) instead of letting app_main.py
                # silently fall back to whichever copy is bundled. Passed as
                # an env var rather than an argv entry so it can't collide
                # with the positional/flag args forwarded to
                # dgiwg_validator.main() below.
                env["DGIWG_VALIDATOR_WORKER_DIR"] = str(vdir)
            else:
