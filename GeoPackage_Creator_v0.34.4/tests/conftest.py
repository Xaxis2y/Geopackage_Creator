...
def _skip_if_gdb_write_unsupported(driver, temp_dir):
    """Skip the calling test if this GDAL build cannot WRITE a File Geodatabase.

    v0.30.6: some GDAL builds (and OpenFileGDB with UseExceptions enabled, which
    the converter turns on process-wide) raise "OGR Error: Corrupt data" when
    creating a .gdb. The GeoPackage Creator only READS .gdb inputs; being unable
    to synthesise one in the test environment is an environment limitation, not
    a product defect, so these fixtures skip rather than emit a spurious ERROR.
    """
    probe_path = os.path.join(temp_dir, "_gdb_write_probe.gdb")
    try:
        srs = osr.SpatialReference()
        srs.ImportFromEPSG(4326)
        ds = driver.CreateDataSource(probe_path)
        layer = ds.CreateLayer("probe", srs, ogr.wkbPoint)
        ...
        pytest.skip(f"OpenFileGDB write unsupported in this GDAL build: {exc}")
...
def temp_dir():
    ...
    yield tmpdir
    # Cleanup.
    # v0.30.6: use ignore_errors=True. On Windows a GDAL datasource that a test
    # left open keeps a lock on the .gpkg/.gdb file, so shutil.rmtree() would
    # raise PermissionError [WinError 32] at teardown and mark the test as an
    # ERROR even though the test body itself passed. Ignoring cleanup errors
    # keeps the OS temp file around briefly (the OS reclaims it) without
    # failing the test.
...
def sample_geodatabase(temp_dir):
    ...
    _skip_if_gdb_write_unsupported(driver, temp_dir)
...
def sample_geopackage(temp_dir, sample_shapefile):
    ...
    # v0.30.6 fix: copy the sample shapefile layer (schema AND features) with
    # CopyLayer. The previous code created an empty "points" layer with no
    # fields and then called CreateFeature(shp_feature); because the target had
    # no matching columns, GDAL raised "table points has no column named ..."
    # and every test using this fixture errored at setup. CopyLayer creates the
    # field definitions before writing features and adds the R-Tree index.
    shp_ds = ogr.Open(sample_shapefile)
    shp_layer = shp_ds.GetLayer(0)
    ds.CopyLayer(shp_layer, "points", ["SPATIAL_INDEX=YES"])
