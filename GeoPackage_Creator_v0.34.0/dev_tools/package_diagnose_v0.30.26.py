...
"""
Packaging automation for diagnose_crash_v0.30.26.py.
...
------------------------------------------------------------------
Run from inside dev_tools\\, alongside diagnose_crash_v0.30.26.py:

    conda activate geopackage
    cd dev_tools
    python package_diagnose_v0.30.26.py

Produces:
    dev_tools\\diagnostics_dist\\diagnose_crash_v0.30.26_<timestamp>\\
        diagnose_crash_v0.30.26.py
        VERSION.txt
    dev_tools\\diagnostics_dist\\diagnose_crash_v0.30.26_<timestamp>.zip
"""
...
SCRIPT_NAME = "diagnose_crash_v0.30.26.py"
DIAGNOSTIC_VERSION = "0.30.26"
...
def main() -> int:
    ...
    version_txt.write_text(
        ...
        "\n"
        "Follow-up to diagnose_crash_v0.30.25.py, which found: real convert()\n"
        "survives single-threaded (P16) but crashes the first time a SECOND\n"
        "distinct OS thread runs it after a first thread already completed\n"
        "one (P17/P18), independent of report generation. No probe before\n"
        "this version ever called stdlib sqlite3, which the real pipeline\n"
        "uses twice per conversion (_embed_metadata, _finalize_dgiwg_\n"
        "compliance). This version checks sqlite3.threadsafety directly,\n"
        "then bisects GDAL / lxml / sqlite3 in pairs across 2 fresh threads.\n"
        "See the script's own module docstring for the full reasoning and\n"
        "how to read the results.\n"
        "\n"
        "Usage: place diagnose_crash_v0.30.26.py in the project's dev_tools\\\n"
        ...
        "\n"
        "Send back dev_tools\\logs\\diagnose_v0.30.26_<timestamp>.log.\n",
        encoding="utf-8",
    )
