...
def _check_xml_xsd(xml_text, xsd_schema):
    ...
    try:
        ...
        # Metadata comes from an untrusted GeoPackage.  Never resolve external
        # entities or access the network while parsing it (XXE/SSRF hardening).
        _parser = _lxml_etree.XMLParser(
            resolve_entities=False, no_network=True, load_dtd=False,
            huge_tree=False, recover=False,
        )
        _lxml_root = _lxml_etree.fromstring(xml_text.strip().encode("utf-8"), _parser)
...
def score_results(
    ...
    counts = {"PASS": 0, "FAIL": 0, "PASS*": 0, "SKIPPED": 0,
              "N/A": 0, "WARNING": 0, "ERROR": 0}
    ...
    passed = counts["PASS"]
    if errors:
        verdict = "VALIDATION ERROR — REVIEW REQUIRED"
        verdict_color = "#8e44ad"
    elif fail == 0 and warn == 0 and counts["SKIPPED"] == 0:
        verdict = "CONFORMANT"
        verdict_color = "#27ae60"
    elif fail == 0 and warn == 0:
        verdict = "AUTOMATED CHECKS CONFORMANT — EXTERNAL REVIEW REQUIRED"
        verdict_color = "#2471a3"
    elif fail == 0:
