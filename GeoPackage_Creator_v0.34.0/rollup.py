...
"""Rollup aggregation + DGIWG_GPKG_FINAL_REPORT.html/.csv (v1.62)."""
import os
import sys
import html
...
def aggregate_rollup(all_results):
    ...
    for filename, results, *_ in all_results:
        ...
        for req_num, r in results.items():
            # v1.62 fix: select requirement rows by key TYPE, not by a
...
def render_rollup_html(
    ...
    for pf, (fname, results, report_fname) in zip(per_file, all_results):
        ...
        file_rows += (
            ...
            f'<td style="padding:6px 10px;color:#8e44ad;">{counts["ERROR"]}</td>'
...
<table>
  <thead><tr>
    <th>File</th><th>Source Software</th><th>Verdict</th><th>PASS</th><th>FAIL</th><th>ERROR</th><th>PASS*</th><th>PASS* (Manual Confirmed)</th><th>SKIPPED</th>
  </tr></thead>
...
<div class="footer">
  ...
  <div style="margin-top:2px;font-size:11px;color:#7f8c8d !important;">&copy; MCE/T&amp;E &mdash; Mapping and Charting Establishment / Geomatics Engineering Trials &amp; Evaluation Support Section (GETESS)</div>
    ...
    with open(output_path, "w", encoding="utf-8") as f:
        f.write(html_str)
    # v1.62 fix: honour --quiet (previously printed unconditionally).
