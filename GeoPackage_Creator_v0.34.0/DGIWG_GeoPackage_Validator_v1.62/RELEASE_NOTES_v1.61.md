...
Previous: v1.60 hardened

v1.62 is a compatibility and release-hardening update. It keeps the v1.60
safety model while correcting scope-sensitive checks that otherwise produce
false failures on valid GeoPackages.

## v1.62 changes

- Req 3 accepts the standard `GPKG` application marker when the schema version
  is valid; legacy GP10/GP11 markers remain failures.
- Recursive discovery validates every `*.gpkg` file, including names such as
  `sample.gpkg.gpkg`.
- Req 5 evaluates registered extensions against their actual table/data scope
  instead of applying gridded-only rules to the whole package.
- Req 13 recognises DGIWG dynamic CRS identifiers in the documented 100100–
  100399 convention range, including URI-form CRS names.
- Req 18 recognises ISO 19115-3/MI metadata and reports an explicit
  `PASS*`/external-ATS review instead of incorrectly applying the bundled
  ISO 19115-2 DMF schema to a different encoding.
- The Anaconda launcher uses a locale-independent timestamp, so logs are
  consistently written as `anaconda_validation_YYYYMMDD_HHMMSS.log`.
- Local regression coverage is 66 assertions and the release package is rebuilt
  from the v1.62 source tree.
