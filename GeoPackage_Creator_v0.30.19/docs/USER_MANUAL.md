...
>
> **Install lxml in the SAME command as GDAL**, as shown below. A pip-
> installed lxml bundles its own libxml2, separate from whatever GDAL
> resolves through conda; that mismatch is a confirmed cause of an access
> violation (v0.30.13) that the tool now detects and refuses to run under
> (`core.config.ALLOW_LIBXML_ABI_MISMATCH`). Verify after installing:
> ```bash
> python -c "from lxml import etree; print(etree.LIBXML_COMPILED_VERSION, etree.LIBXML_VERSION)"
> ```
> The two tuples must match.
...
**Source .gdb** — click Browse. For File Geodatabases, select the `.gdb` *folder itself* (do not double-click into it). Shapefiles and GeoJSON can be selected as files.
...
```bash
python DGIWG_GeoPackage_Validator_v1.58/DGIWG_Validator_v1_58.py output.gpkg
```
...
**Source .gdb not recognized**
Select the `.gdb` folder itself, not a file inside it. Verify with `ogrinfo yourdata.gdb`.
