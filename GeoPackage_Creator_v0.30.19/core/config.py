...
GPKG_APPLICATION_ID_DGIWG = 0x47503134
"""OGC/DGIWG application_id marker ('GP14' in hex: 0x47503134).

OGC GeoPackage encodes the spec version directly in the SQLite header
application_id field (e.g. GP12=1.2, GP13=1.3, GP14=1.4). This value must
be consistent with GPKG_USER_VERSION (10400 = 1.4); a mismatch makes the
file simultaneously claim two different versions and fails OGC validation.

DGIWG STD-DP-19-005 Req 3 mandates GeoPackage 1.2 or later — GP14 satisfies
that requirement since 1.4 >= 1.2. The bundled DGIWG validator (checks.py
_r3) is updated to accept GP14 alongside GP12/GP13 as conformant.

GDAL writes 0x47504B47 ('GPKG'), which is a non-standard marker that predates
the OGC version-encoding scheme; the finalization step overwrites it with the
correct version-specific value.
"""
