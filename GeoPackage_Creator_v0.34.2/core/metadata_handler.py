...
_VALIDATOR_SCRIPT = Path(__file__).with_name("schema_validation_worker.py")
...
class _IsolatedSchemaValidator:
    """Compatibility facade for ``MetadataHandler.schema``.

    It deliberately holds no lxml object.  ``validate`` launches a helper
    process which imports lxml but never imports GDAL, preventing the native
    GDAL/libxml2 interaction observed on Windows.
    """

    def __init__(self, schema_path: Path):
        self.schema_path = schema_path

    def validate(self, metadata: Union[str, bytes]) -> bool:
        if isinstance(metadata, str):
            metadata = metadata.encode("utf-8")
        if not isinstance(metadata, bytes):
            raise TypeError("metadata must be str or bytes")
        try:
            command = [sys.executable]
            if getattr(sys, "frozen", False):
                command.append("--schema-validation-worker")
            else:
                command.append(str(_VALIDATOR_SCRIPT))
            command.append(str(self.schema_path))
            proc = subprocess.run(
                command,
                input=metadata,
                capture_output=True,
            ...
            raise ValueError("Schema validation helper timed out after 60 seconds") from exc
        except OSError as exc:
            raise ValueError(f"Could not start schema validation helper: {exc}") from exc
        ...
        try:
            ...
            raise ValueError(
                f"Schema validation helper failed (exit {proc.returncode}): "
                f"{stderr or 'invalid response'}"
            ) from exc
        if proc.returncode == 0 and reply.get("ok") is True:
            return True
        raise ValueError(str(reply.get("error") or "Schema validation failed"))
...
class MetadataHandler:
    ...
    @property
    def schema(self) -> Optional[_IsolatedSchemaValidator]:
        ...
        return _IsolatedSchemaValidator(schema_path) if schema_path else None

    def _load_iso19115_schema(self) -> Optional[_IsolatedSchemaValidator]:
        """
    ...
    def validate_schema(self, metadata_xml_string: str) -> bool:
        ...
        v0.30.23: the schema compile, the parse, the validate, and the
        error_log read now ALL happen under one acquisition of the module
        lock - previously (v0.30.10-v0.30.22) only parse/validate/error_log
        did, because the schema was compiled once at construction and simply
        reused. It is now compiled fresh on this very call (see
        `_compile_schema_fresh()` and the module docstring's "SCHEMA
        LIFETIME VS GDAL WRITES" section), so its entire lifetime - compile,
        use, drop - must be contained in this one call and this one lock
        acquisition; it is a purely local variable, never stored on `self`
        or anywhere else, so nothing can hold it open across a later GDAL
        write.
        ...
        if not schema:
            ...
            return True
        result = schema.validate(metadata_xml_string)
        logger.info("ISO 19115 schema validation passed in isolated helper")
        return result
