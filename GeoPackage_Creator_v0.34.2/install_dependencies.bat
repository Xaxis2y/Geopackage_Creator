...
REM Step 2: Install GUI theming (pip only - not on conda-forge)
echo [2/3] Installing GUI theming (ttkbootstrap)...
...
if %ERRORLEVEL% equ 0 (
    ...
    echo.
    echo [WARNING] Failed to install ttkbootstrap - the GUI will not start
    echo without it. You can retry later with:
    echo   python -m pip install "ttkbootstrap==2.2.0"
)

echo.

REM Step 3: Install GDAL
echo [3/3] Installing GDAL...
...
echo   Then run: conda install -c conda-forge gdal=3.13.2
echo.
