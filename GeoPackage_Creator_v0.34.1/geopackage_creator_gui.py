...
"""
GeoPackage Creator v0.34 - Enhanced GUI
...
def is_file_geodatabase_folder(path: str) -> bool:
    """Return whether *path* is an ESRI File Geodatabase directory."""
    folder = Path(path)
    return folder.is_dir() and (
        folder.suffix.lower() == ".gdb" or any(folder.glob("GEODATABASE_FILE_*"))
    )


def resolved_file_geodatabase_path(path: str) -> str:
    """Return a directly openable .gdb path for a CANVEC-style wrapper."""
    folder = Path(path)
    candidates = sorted(folder.glob("GEODATABASE_FILE_*/*.gdb")) if folder.is_dir() else []
    return str(candidates[0]) if len(candidates) == 1 else path
...
class GeoPackageCreatorGUI:
    """Enhanced GUI application for GeoPackage Creator v0.34."""
    ...
    def create_file_selection_section(self, parent):
        ...
        # Source GDB
        ttk.Label(file_frame, text="Source FileGDB:").grid(row=0, column=0, sticky="w")
    ...
    def browse_source(self):
        ...
        folder = filedialog.askdirectory(title="Select source File Geodatabase folder", initialdir=str(Path.home() / "Documents"))
        if folder:
            if not is_file_geodatabase_folder(folder):
                messagebox.showwarning(
                    "Warning",
                    "Selected folder is not recognized as an ESRI File Geodatabase.\n"
                    "Choose a *.gdb folder or one containing GEODATABASE_FILE_*.",
                )
                return
            source = resolved_file_geodatabase_path(folder)
    ...
    def validate_inputs(self):
        ...
        if not self.source_var.get():
            errors.append("Source File Geodatabase folder is required")
        elif not is_file_geodatabase_folder(self.source_var.get()):
            errors.append(
                "Source is not recognized as an ESRI File Geodatabase. "
                "Select a *.gdb folder or a folder containing GEODATABASE_FILE_*."
            )
