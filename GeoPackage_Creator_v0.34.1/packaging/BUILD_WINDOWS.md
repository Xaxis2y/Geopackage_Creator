...
- **`GeoPackageCreator.spec`** collects, into the bundle:
  ...
  - `DGIWG_GeoPackage_Validator_v1.62/` (the `--validate` engine)
...
| `--validate` says validator not found | Confirm `DGIWG_GeoPackage_Validator_v1.62\dgiwg_validator\checks.py` was bundled; `runtime_paths.py` sets `DGIWG_VALIDATOR_PATH`. |
