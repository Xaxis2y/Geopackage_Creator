...
class GeoPackageConverter:
        ...
        try:
            ...
            try:
                ...
                with GDALHandler() as handler:
                    ...
                    handler.close_geopackage(out_ds)
            ...
            #   * sets application_id to the DGIWG 'GP12' marker (0x47503132)
    ...
    def _embed_metadata(
        ...
        try:
            ...
            try:
                ...
                for ext_table in ("gpkg_metadata", "gpkg_metadata_reference"):
                    cur.execute(
                        """
                        INSERT INTO gpkg_extensions
                            (table_name, column_name, extension_name,
                             definition, scope)
                        SELECT ?, NULL, 'gpkg_metadata', ?, 'read-write'
                        WHERE NOT EXISTS (
                        ...
                        """,
                        (ext_table, extension_def, ext_table),
                    )
    ...
    def _wkt2_for_epsg(epsg: int) -> Optional[str]:
        """Return an authoritative WKT2:2019 string for an EPSG code.

        Preference order:
          1. pyproj - produces exactly the WKT2 the DGIWG validator parses
             (correct lat-first axis order for geographic CRS, ENSEMBLE/DATUM
             names that match the EPSG registry).
          2. GDAL osr - ExportToWkt(FORMAT=WKT2_2019); always available since
             GDAL is the converter's core dependency.
        Returns None if neither can produce a string.
        """
        try:
            from pyproj import CRS as _PyCRS  # type: ignore
            return _PyCRS.from_epsg(epsg).to_wkt()  # default = WKT2_2019
        except Exception:
            pass
        try:
        ...
        try:
            ...
            for tbl, col in (("gpkg_contents", "srs_id"),
                ...
                try:
                    used_srs |= {
                        r[0] for r in
                        cur.execute(f"SELECT {col} FROM {tbl}").fetchall()
                        if r[0] is not None
                    }
            ...
            # 7) Set the DGIWG 'GP12' application_id marker. Done last so no
            #    later write resets it. user_version (10400) is left intact.
            cur.execute(f"PRAGMA application_id = {GPKG_APPLICATION_ID_DGIWG}")
            conn.commit()
            info["application_id_set"] = True

            # 8) Req 24 fix: recalculate gpkg_contents bbox from RTree aggregates.
            # GDAL may write a slightly imprecise bbox at layer-creation time;
            # pulling the exact min/max from the RTree guarantees a match.
            logger.info("Req 24 fix: recalculating gpkg_contents bbox from RTree...")
            info["bbox_fixed_layers"] = self._fix_bbox_from_rtree(cur)
            conn.commit()

            logger.info(
                "DGIWG finalize: app_id=0x%08X, WKT2 for %s, orphan CRS removed %s, "
                "bbox fixed for %s",
                GPKG_APPLICATION_ID_DGIWG,
                info["wkt2_written_for"],
                info["orphan_crs_removed"],
                info["bbox_fixed_layers"],
            )
        finally:
            conn.close()

        # 9) Req 24 fix: strip residual Z flags from geometries in 2D-declared layers.
        # Must run AFTER the sqlite3 connection is closed so GDAL can open for update.
        logger.info("Req 24 fix: stripping residual Z flags from 2D-declared layers...")
        info["z_stripped_layers"] = self._strip_z_from_2d_layers(gpkg_path)
        if info["z_stripped_layers"]:
            logger.info(
                "Z-strip complete: %s layer(s) fixed",
                len(info["z_stripped_layers"]),
            )
    ...
    def get_active_profile_config(self) -> Dict[str, Any]:
        """
        Get configuration of the active conversion profile.

        Returns:
            Dict with profile settings (security_level, language,
            topic_category, spatial_index)
        """
