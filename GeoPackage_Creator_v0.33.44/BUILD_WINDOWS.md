...
This source tree does not currently include an Inno Setup `.iss` installer
definition. Distribute the portable one-file executable or the one-directory
bundle above, or add and maintain a project-specific installer definition
before promising a `Setup.exe` release.
...
- **`GeoPackageCreator.spec`** collects, into the bundle:
  ...
  - `DGIWG_GeoPackage_Validator_v1.58/` (the `--validate` engine)
...
| `--validate` says validator not found | Confirm `DGIWG_GeoPackage_Validator_v1.58\dgiwg_validator\checks.py` was bundled; `runtime_paths.py` sets `DGIWG_VALIDATOR_PATH`. |
