...
"""Requirement check functions for DGIWG STD-DP-19-005 v1.1 (v1.58).
...
def _r3(cursor):
    ...
    try:
        # GeoPackage 1.2+ has a fixed GPKG application id; user_version holds
        # the encoded specification version (OGC 12-128r19 Req 2).
        ...
        if _app_id == 0x47504B47:
            if _user_version >= 10200:
                passes.append(
                    f"GeoPackage version {(_user_version // 10000)}."
                    f"{((_user_version // 100) % 100):02d}."
                    f"{(_user_version % 100):02d} (application_id=GPKG) ✓"
                )
            else:
                fails.append(
                    f"application_id=GPKG but user_version={_user_version}; "
                    "DGIWG requires GeoPackage 1.2 or later"
                )
        elif _app_id in _GP_VERS:
            ...
            fails.append(
                ...
                f"expected 0x47504B47 (GPKG, GeoPackage 1.2+)"
            )
...
def _r4(cursor):
    ...
    # exception FAIL.  Optional extensions can never fail by definition —
    # no table means none are registered, which is fully conformant.
    if not table_exists(cursor, "gpkg_extensions"):
