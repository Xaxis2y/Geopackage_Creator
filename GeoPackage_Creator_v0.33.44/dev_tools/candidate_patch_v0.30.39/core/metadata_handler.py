...
PERSISTENT WORKER SHUTDOWN (v0.30.25 - ISOLATING A SECOND, DIFFERENT CRASH)
--------------------------------------------------------------------------
dev_tools/validate_patch_v0.30.38.py ran P60/P61 on the target machine on
2026-08-17. P60 (unpatched control) reproduced the original crash exactly as
every prior control has - one round completes, the process dies before round
1 even starts. P61 (this file's v0.30.24 candidate, applied to a temporary
project copy) did NOT reproduce that failure shape: the log shows all 10
rounds completing ("round 0" through "round 9", each printing "thread
completed its FULL, real, unmodified conversion"), and the probe's own
closing line - "P61 OK - all 10 sequential rounds completed... zero crashes"
- is present in the captured output. Only AFTER that line does the process
exit with the same access-violation code (3221225477) P60 produces. The
"P61 setup" lines confirm the staged copy's __version__ read "0.30.24" before
the run proceeded, so this is not a case of the patch failing to apply.

Put plainly: every real instance of the exact pattern this whole series has
been chasing - thread A converts, is joined, thread B is created and
converts - completed with zero crashes, at 2x the repetition already
validated under monkey-patching (`diagnose_crash_v0.30.35.py`'s P55, 5
rounds). The crash in that run happened once the Python-level work was
already finished, during process teardown - a different failure shape from
every P17-P60 control, all of which crash mid-pattern, not after it.

v0.30.24's worker thread (`_ensure_lxml_worker_started()` above) is started
with `daemon=True` and its body, `_lxml_worker_loop()`, is an unconditional
`while True` blocked on `_LXML_JOBS.get()` with no timeout. Nothing in
v0.30.24 - no sentinel, no `.join()`, no `atexit` hook - ever asks that
thread to stop. So when a short-lived process (`validate_patch_v0.30.38.py`'s
P61 subprocess; equally, a real CLI invocation, or a user closing the GUI
right after converting) finishes its work and lets the interpreter exit
normally, this thread is still alive, parked in a blocking call, exactly
when CPython finalization and then Windows' own DLL-unload sequence for
libxml2.dll and the CRT run. A live thread that has previously touched
libxml2 state, caught by that teardown at an arbitrary point, is a
plausible - not yet confirmed - fit for the same broad category this whole
investigation has already named as an open question in
`dev_tools/CRASH_INVESTIGATION_STATUS_2026-08-17.md`: "something thread-
identity-scoped... a thread-local allocator arena, a TLS-indexed cache
inside libxml2 or the CRT." It is also worth noting, only as circumstantial
support and not as evidence about THIS mechanism, that this file's own
v0.30.13 section already used the phrase "during interpreter shutdown with
no Python traceback at all" to describe a failure category this project has
hit once before, for an unrelated defect (the ABI mismatch).

This version does not claim to have confirmed that hypothesis, and does not
change anything about when or how the worker thread is created, or its
`daemon=True` setting. It adds exactly one new capability, unused by any
caller in this file: `shutdown_lxml_worker()`, which puts a sentinel
(`_LXML_WORKER_STOP`) onto the job queue and joins the worker thread with a
timeout. `_lxml_worker_loop()` checks for that sentinel, by identity, before
unpacking a job, and returns (ending the thread) when it sees it. Nothing in
this file calls `shutdown_lxml_worker()` automatically yet - no `atexit`
hook is registered, and no GUI/CLI integration is wired up. It exists so
`dev_tools/validate_patch_shutdown_v0.30.39.py` can call it explicitly from a
test harness and isolate whether an explicit, joined shutdown before process
exit avoids the crash seen in P61. Whether to wire this in automatically
(and how - `atexit`, an explicit GUI close handler, something else), and
whether it is even the right fix, are separate, later decisions once that
script's result is in - consistent with how every other change in this
module has been proven on the target machine before being proposed further.
"""
...
#
# v0.30.25 adds `_LXML_WORKER_STOP` and `shutdown_lxml_worker()` - see the
# module docstring's "PERSISTENT WORKER SHUTDOWN" section. Nothing about how
# or when the thread STARTS changes in this version.
# ---------------------------------------------------------------------------
...
# The job queue the worker thread consumes. Each job is EITHER a
# (handler, xml_bytes, response_queue) tuple, put only by `validate_schema()`
# below, OR the `_LXML_WORKER_STOP` sentinel, put only by
...
# v0.30.25: a unique sentinel object, compared by identity (`is`), never by
# equality - so it can never be confused with a real
# (handler, xml_bytes, response_q) job tuple, however that tuple's contents
# happen to compare. Only `shutdown_lxml_worker()` ever puts this onto
# `_LXML_JOBS`; only `_lxml_worker_loop()` ever checks for it.
_LXML_WORKER_STOP = object()
...
def _ensure_lxml_worker_started() -> None:
    ...
    Unchanged behaviourally since v0.30.24. If `shutdown_lxml_worker()` has
    previously stopped the worker (`_LXML_WORKER_THREAD` reset to `None`),
    this function starts a brand-new thread exactly as it would on first
    use - the persistent worker is restartable, not a strictly one-shot
    singleton, though nothing in this file exercises that restart during
...
def shutdown_lxml_worker(timeout: float = 5.0) -> bool:
    """Best-effort, explicit stop-and-join of the persistent lxml worker
    thread (v0.30.25).

    Not called from anywhere in this file. Added so a caller - today, only
    `dev_tools/validate_patch_shutdown_v0.30.39.py`'s test harness - can ask
    the worker thread to exit cleanly and wait for it, instead of leaving it
    to be cut off by process exit. See the module docstring's "PERSISTENT
    WORKER SHUTDOWN" section for why this exists: `dev_tools/validate_patch_
    v0.30.38.py`'s P61 run completed all 10 rounds of real, unmodified
    conversions with zero crashes, then crashed with the same access-
    violation code once the interpreter began finalizing - at which point
    this thread, started with `daemon=True` and never stopped, was still
    alive and blocked in `_LXML_JOBS.get()`. Whether joining it first avoids
    that crash is exactly what calling this function is meant to test; this
    function does not itself confirm or assume an answer.

    Safe to call from any thread, any number of times, including when the
    worker was never started at all, or has already been stopped - both are
    treated as "nothing to do," not an error.

    This function does not coordinate with concurrent `validate_schema()`
    calls made from OTHER threads while it runs: a job queued at nearly the
    same moment as the stop sentinel may be picked up only once a new worker
    is started again (by a later `validate_schema()` call, via
    `_ensure_lxml_worker_started()`), or may time out first (see
    `_LXML_WORKER_RESPONSE_TIMEOUT`). `dev_tools/validate_patch_shutdown_
    v0.30.39.py` only calls this function after every conversion thread has
    already been joined and no further `validate_schema()` calls are in
    flight, so that window is not exercised by that script. Closing it
    properly - if it needs closing at all, given the shipped GUI's single-
    conversion-at-a-time model - is a separate concern for whoever
    eventually wires this into a real shutdown path.

    Args:
        timeout: seconds to wait for the worker thread to exit after the
            stop sentinel is queued, before giving up and returning False.
            The module docstring's rationale for `_LXML_WORKER_RESPONSE_
            TIMEOUT` applies here too - a real validation is on the order of
            a millisecond, so 5 seconds is a generous ceiling, not a
            realistic expected wait.

    Returns:
        True if the worker thread was not running (nothing to do) or has
        now exited cleanly. False if a worker thread was running and did
        not exit within `timeout` seconds - the caller should treat this as
        informational; it does not undo the stop request, and the thread
        may still exit shortly after this function returns.
    """
    global _LXML_WORKER_THREAD

    with _LXML_WORKER_STARTUP_LOCK:
        t = _LXML_WORKER_THREAD
        if t is None:
            return True
        if not t.is_alive():
            _LXML_WORKER_THREAD = None
            return True
        _LXML_JOBS.put(_LXML_WORKER_STOP)

    t.join(timeout=timeout)
    stopped = not t.is_alive()
    if stopped:
        with _LXML_WORKER_STARTUP_LOCK:
            if _LXML_WORKER_THREAD is t:
                _LXML_WORKER_THREAD = None
    else:
        logger.warning(
            "shutdown_lxml_worker(): worker thread did not exit within "
            f"{timeout}s of receiving the stop sentinel."
        )
    return stopped
...
def _lxml_worker_loop() -> None:
    """Body of the persistent lxml worker thread.

    v0.30.25: runs until it receives the `_LXML_WORKER_STOP` sentinel via
    `shutdown_lxml_worker()` - normally never, since nothing in this file
    calls that function automatically. Before v0.30.25 this loop never
    returned at all; see the module docstring's "PERSISTENT WORKER
    SHUTDOWN" section for why an explicit stop path was added and what it
    does and does not prove by itself.

    Otherwise unchanged from v0.30.24: pulls one job at a time from
    `_LXML_JOBS` and runs it through `_validate_schema_impl()`. This is the
    ONLY place in the process that ever calls `_validate_schema_impl()` -
    which is exactly what makes it the only thread that ever touches lxml
    ...
    while True:
        job = _LXML_JOBS.get()
        if job is _LXML_WORKER_STOP:
            logger.debug(
                "Persistent lxml worker thread received stop sentinel "
                "(v0.30.25) - exiting cleanly."
            )
            return
